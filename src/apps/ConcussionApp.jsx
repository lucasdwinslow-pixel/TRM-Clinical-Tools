...
const MUTED    = "#555555";
const WHITE    = "#ffffff";
// glass-morphism tokens (on-screen reskin only — PDF is drawn separately)
const GLASS = "rgba(28,32,35,0.6)";
const GLASS_DIM = "rgba(11,15,18,0.6)";
...
// ─── SHARED STYLES ────────────────────────────────────────────────────────────
const inp = {
  background: GLASS_DIM, border: `1px solid ${GLASS_BORDER}`, borderRadius: 8,
...
function Card({ title, accent, children, id, focusable, activeCard, setActiveCard }) {
  ...
  return (
    <div id={id} style={{
      background: GLASS, backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", borderRadius: 14, marginBottom: 20, overflow: "hidden",
      border: `1px solid ${accent ? LIME + "44" : isActive ? LIME + "66" : GLASS_BORDER}`,
      boxShadow: accent ? `0 0 24px ${LIME}18` : isActive ? `0 0 20px ${LIME}22` : "0 8px 32px rgba(0,0,0,0.37)",
      transition: "box-shadow 0.2s, border-color 0.2s",
    }}>
      <div onClick={handleClick} style={{
        padding: "12px 20px",
        background: accent ? `linear-gradient(90deg,${LIME}18,transparent)` : isActive ? `linear-gradient(90deg,${LIME}14,transparent)` : `linear-gradient(90deg,${LIME}0d,transparent)`,
        borderBottom: `1px solid ${accent ? LIME + "33" : isActive ? LIME + "33" : GLASS_BORDER}`,
...
function Field({ label, value, onChange, type = "number", step = "1", placeholder = "—", unit, readOnly }) {
  return (
    <div>
      ...
      <input
        style={readOnly ? { ...inp, color: LIME, background: "rgba(11,15,18,0.5)", borderColor: LIME + "33", cursor: "default" } : inp}
...
function StatBar({ stats }) {
  return (
    <div className="trm-con-stat-bar" style={{ background: "rgba(11,15,18,0.5)", border: `1px solid ${GLASS_BORDER}`, borderRadius: 10, padding: "12px 20px", display: "flex", gap: 28, flexWrap: "wrap", marginTop: 8 }}>
      {stats.map((s, i) => (
...
async function saveSessionPDF(data) {
  ...
  const BGRAY    = rgb(0.11, 0.11, 0.11);
  const BORDER_R = rgb(0.165, 0.165, 0.165);
  ...
  const draw = (text, x, yp, size, f, c) => {
    try { page.drawText(sanitizePdf(text), { x, y: yp, size, font: f || font, color: c || BLACK_R }); } catch(e){}
  };
  ...
  const colW = (R - L) / Math.max(ptFields.length, 1);
  ptFields.forEach(([lbText, val], i) => {
    const px = L + i * colW;
    try {
      page.drawText(sanitizePdf(lbText.toUpperCase()), { x: px, y: height - 88, size: 6.5, font: fontBold, color: rgb(0.44, 0.44, 0.44) });
      page.drawText(sanitizePdf(String(val)), { x: px, y: height - 100, size: 8.5, font: fontBold, color: WHITE_R });
    } catch(e) {}
  });
  ...
  // ── Content ──
  const sections = buildConcussionNote(data).split("\n");
  for (const rawLine of sections) {
    ...
    if (isHeader) {
      y -= 4;
      page.drawRectangle({ x: L - 4, y: y - 3, width: R - L + 8, height: 14, color: rgb(0.14, 0.14, 0.14) });
      draw(rawLine.trim(), L, y, 7.5, fontBold, LIME_R);
      y -= 17;
    } else {
      const isBullet = rawLine.startsWith("  ");
      const xOff = isBullet ? L + 10 : L;
      const maxW = R - xOff;
      const words = sanitizePdf(rawLine.trim()).split(" ");
      let cur = "";
      const lines = [];
      for (const w of words) {
        const test = cur ? cur + " " + w : w;
        try {
          if (font.widthOfTextAtSize(test, 9) <= maxW) { cur = test; }
      ...
      for (const wl of lines) {
        if (y < 40) break;
        try { page.drawText(wl, { x: xOff, y, size: 9, font, color: BLACK_R }); } catch(e){}
        y -= 13;
      }
    }
  }
  ...
  draw("TRM  |  Concussion / mTBI Evaluation Tool", L, 8, 6.5, font, rgb(0.44, 0.44, 0.44));
  draw("Page 1", R - font.widthOfTextAtSize("Page 1", 6.5), 8, 6.5, font, rgb(0.44, 0.44, 0.44));

  // ── Page 2: documentation note ──
  const noteText = buildConcussionNote(data);
  const page2 = doc.addPage([612, 792]);
  const { width: w2, height: h2 } = page2.getSize();
  const L2 = 48, R2 = 564;

  const draw2 = (text, x, yp, size, f, c) => {
    try { page2.drawText(sanitizePdf(text), { x, y: yp, size, font: f || font, color: c || BLACK_R }); } catch(e){}
  };
  ...
  for (const rawLine of noteLines) {
    ...
    if (isHeader) {
      y2 -= 4;
      page2.drawRectangle({ x: L2 - 4, y: y2 - 3, width: R2 - L2 + 8, height: 14, color: rgb(0.14, 0.14, 0.14) });
      try { page2.drawText(sanitizePdf(rawLine.trim()), { x: L2, y: y2, size: 7.5, font: fontBold, color: LIME_R }); } catch(e){}
      y2 -= 17;
    } else {
      const xOffset = isBullet ? L2 + 10 : L2;
      const words2 = sanitizePdf(rawLine.trim()).split(" ");
      let cur2 = "";
      const wrapped = [];
      for (const w of words2) {
        const test = cur2 ? cur2 + " " + w : w;
        try {
          if (font.widthOfTextAtSize(test, 9) <= maxLineWidth - (isBullet ? 10 : 0)) { cur2 = test; }
          else { if (cur2) wrapped.push(cur2); cur2 = w; }
        } catch(e) { cur2 = test; }
      }
      if (cur2) wrapped.push(cur2);
      for (const wl of (wrapped.length ? wrapped : [""])) {
        if (y2 < 40) break;
        try { page2.drawText(wl, { x: xOffset, y: y2, size: 9, font, color: BLACK_R }); } catch(e){}
        y2 -= 13;
      }
    }
  }
  ...
  // Page 2 footer
  page2.drawRectangle({ x: 0, y: 0, width: w2, height: 26, color: DARK_R });
  page2.drawRectangle({ x: 0, y: 26, width: w2, height: 1.5, color: LIME_R });
  draw2("TRM  |  Concussion / mTBI Evaluation Tool", L2, 8, 6.5, font, rgb(0.44, 0.44, 0.44));
  draw2("Page 2 — EMR Copy", R2 - font.widthOfTextAtSize("Page 2 — EMR Copy", 6.5), 8, 6.5, font, rgb(0.44, 0.44, 0.44));
...
export default function App() {
  ...
  return (
    <div style={{ backgroundColor: "#0b0f12", backgroundImage: "radial-gradient(circle at 0% 0%, rgba(184,255,87,0.05) 0%, transparent 42%), radial-gradient(circle at 100% 100%, rgba(184,255,87,0.05) 0%, transparent 42%)", backgroundAttachment: "fixed", minHeight: "100vh", color: WHITE, fontFamily: "'Inter','Helvetica Neue',sans-serif" }}>
      ...
      {/* Header */}
      <div style={{ background: "rgba(11,15,18,0.78)", backdropFilter: "blur(14px)", WebkitBackdropFilter: "blur(14px)", borderBottom: `1px solid ${GLASS_BORDER}`, position: "sticky", top: 0, zIndex: 100, boxShadow: "0 2px 20px rgba(0,0,0,0.8)" }}>
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 20px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: 64 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <span className="trm-con-header-subtitle" style={{ fontSize: 8, fontWeight: 800, color: "rgba(255,255,255,0.3)", letterSpacing: "0.2em", textTransform: "uppercase" }}>Concussion Testing</span>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <input ref={fileInputRef} type="file" accept=".pdf" style={{ display: "none" }} onChange={handleFileChange} />
            </div>
          </div>
          <div style={{ display: "flex", borderTop: `1px solid ${GLASS_BORDER}` }}>
            {tabs.map((t, i) => (
      ...
      <div style={{ borderTop: `1px solid ${GLASS_BORDER}`, padding: "16px 20px", textAlign: "center" }}>
        <svg viewBox="0 0 867 352" height={12} fill={WHITE} role="img" aria-label="TRM" style={{ display: "inline-block", verticalAlign: "middle" }}><path fillRule="evenodd" d="M541.00,4.00 L495.00,278.50 L546.00,346.50 L561.50,345.50 L593.00,144.50 L650.00,346.50 L697.50,345.50 L785.50,144.50 L786.00,348.50 L863.50,348.50 L859.50,4.00 L776.00,5.00 L685.50,202.00 L623.50,4.00 Z M270.00,4.00 L243.00,348.50 L321.50,347.50 L332.00,212.50 L426.00,348.50 L525.50,348.50 L419.50,207.50 L458.50,185.50 L476.50,166.50 L488.50,145.50 L496.50,115.50 L497.50,84.00 L492.50,61.00 L482.50,42.00 L456.50,19.00 L424.50,7.00 L396.50,4.00 Z M344.00,66.50 L371.50,66.50 L372.00,67.50 L379.50,67.50 L380.00,68.50 L383.50,68.50 L384.00,69.50 L388.50,69.50 L389.00,70.50 L391.50,70.50 L394.00,72.50 L396.50,72.50 L397.00,73.50 L398.50,73.50 L400.00,75.50 L401.50,75.50 L408.00,82.00 L408.00,83.50 L409.00,84.00 L409.00,85.50 L410.00,86.00 L410.00,87.50 L411.00,88.00 L411.00,89.50 L413.00,92.00 L413.00,95.50 L414.00,96.00 L414.00,101.50 L415.00,102.00 L415.00,110.50 L414.00,111.00 L414.00,119.50 L413.00,120.00 L413.00,123.50 L412.00,124.00 L412.00,126.50 L411.00,127.00 L411.00,129.50 L410.00,130.00 L409.00,133.50 L407.00,135.00 L407.00,136.50 L405.00,138.00 L405.00,139.50 L396.50,148.00 L395.00,148.00 L394.50,149.00 L393.00,149.00 L392.50,150.00 L391.00,150.00 L388.50,152.00 L383.00,153.00 L382.50,154.00 L379.00,154.00 L378.50,155.00 L375.00,155.00 L374.50,156.00 L369.00,156.00 L368.50,157.00 L337.00,157.00 L336.50,156.50 L336.50,145.00 L337.50,144.50 L337.50,132.00 L338.50,131.50 L338.50,119.00 L339.50,118.50 L339.50,106.00 L340.50,105.50 L340.50,94.00 L341.50,93.50 L341.50,81.00 L342.50,80.50 L342.50,68.00 Z M9.00,4.00 L4.00,72.50 L85.00,73.00 L64.00,348.50 L141.50,348.50 L163.50,73.00 L245.50,72.50 L250.50,4.00 Z"/></svg>
      ...
      <div className="trm-con-fab" style={{ position: "fixed", bottom: 24, right: 24, zIndex: 200, display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <div style={{ display: "flex", alignItems: "stretch", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 8, overflow: "hidden", background: "rgba(255,255,255,0.05)", boxShadow: "0 2px 10px rgba(0,0,0,0.5)" }}>
          <button onClick={() => setNewPtModal(true)} style={{ padding: "8px 12px", background: "rgba(248,113,113,0.09)", color: "rgba(248,113,113,0.85)", border: "none", cursor: "pointer", fontSize: 9, fontWeight: 800, letterSpacing: "0.07em", textTransform: "uppercase" }}>
            Reset
          </button>
          <div style={{ width: 1, background: "rgba(255,255,255,0.12)", flexShrink: 0 }} />
          <button onClick={() => fileInputRef.current.click()} style={{ padding: "8px 12px", background: "transparent", color: "rgba(255,255,255,0.55)", border: "none", cursor: "pointer", fontSize: 9, fontWeight: 800, letterSpacing: "0.07em", textTransform: "uppercase" }}>
            Load
          </button>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "stretch", border: `1px solid ${LIME}52`, borderRadius: 8, overflow: "hidden", background: LIME + "0f", boxShadow: `0 2px 10px ${LIME}14`, opacity: saving ? 0.5 : 1 }}>
          <button onClick={handleSave} disabled={saving} style={{ padding: "8px 14px", background: "transparent", color: LIME + "f2", border: "none", cursor: saving ? "default" : "pointer", fontSize: 9, fontWeight: 800, letterSpacing: "0.07em", textTransform: "uppercase" }}>
