...
if (typeof document !== "undefined" && !document.getElementById("trm-mobile-styles")) {
  ...
  s.textContent = `
    html, body {
      ...
      background: #0a0a0a;
      background-image:
        radial-gradient(circle at 0% 0%, rgba(184,255,87,0.03) 0%, transparent 40%),
        radial-gradient(circle at 100% 100%, rgba(184,255,87,0.03) 0%, transparent 40%);
      background-attachment: fixed;
    }
    /* Card — flat solid, matches Shoulder/Elbow/APRE/Hip */
    .trm-glass {
      background: #181818;
    ...
    input:focus, select:focus, textarea:focus {
      ...
      border-width: 1px !important;
      box-shadow: 0 0 0 1px rgba(184,255,87,0.2) !important;
    }
    /* Scrollbar */
    ::-webkit-scrollbar { width: 6px; height: 6px; }
    ::-webkit-scrollbar-track { background: transparent; }
    ::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.1); border-radius: 10px; }
    ::-webkit-scrollbar-thumb:hover { background: rgba(184,255,87,0.3); }
    input::-webkit-outer-spin-button,
    input::-webkit-inner-spin-button { -webkit-appearance: none; margin: 0; }
    /* Card bar accent */
    .trm-card-bar { width: 3px; height: 16px; border-radius: 2px; flex-shrink: 0; }
        ...
        <div style={{ padding: "28px 28px 24px" }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button
              ...
              style={{
                flex: 1, padding: "11px 0", borderRadius: 10,
                border: `1px solid ${BORDER}`, background: "#181818",
                color: MUTED, fontSize: 12, fontWeight: 700, cursor: "pointer",
                letterSpacing: "0.06em",
              }}>
              Cancel
  ...
  return (
    <div>
      ...
      <Card title="Progress Tracking" accent>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          <button onClick={onAddSession} style={{ padding: "8px 16px", borderRadius: 8, border: `1px solid ${BORDER}`, background: "#181818", color: "#aaa", cursor: "pointer", fontSize: 11, fontWeight: 700 }}>
            + Add Session PDF
          </button>
          {sessions.length > 0 && (
            <button onClick={() => setSessions([])} style={{ padding: "8px 16px", borderRadius: 8, border: `1px solid ${RED_BAD}44`, background: "transparent", color: RED_BAD, cursor: "pointer", fontSize: 11, fontWeight: 700 }}>
          ...
          {/* Session chips */}
          {sessions.map((s, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 10px", borderRadius: 6, background: "#181818", border: `1px solid ${BORDER}` }}>
              <span style={{ fontSize: 10, fontWeight: 700, color: "#aaa" }}>{s.label}</span>
              <button onClick={() => setSessions(prev => prev.filter((_, idx) => idx !== i))} style={{ background: "none", border: "none", color: MUTED, cursor: "pointer", fontSize: 12, lineHeight: 1, padding: 0 }}>×</button>
            </div>
          ))}
      ...
      {/* ── Multi-session comparison table ── */}
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, overflow: "hidden", marginBottom: 20 }}>
        <div style={{ padding: "12px 20px", background: "#181818", borderBottom: `1px solid ${BORDER}`, display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 3, height: 18, borderRadius: 2, background: LIME }} />
          <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: "0.18em", color: MUTED, textTransform: "uppercase" }}>Session Timeline</span>
          {!hasSessions && <span style={{ fontSize: 11, color: MUTED, marginLeft: 8 }}>— Load session PDFs to enable multi-session comparison</span>}
        </div>
        ...
        <div style={{ padding: "16px 20px" }}>
          <div style={{ position: "relative", display: "flex", alignItems: "flex-end", gap: 0 }}>
            ...
            {timelineItems.map((t, i) => {
              ...
              return (
                <div key={t.label} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", position: "relative", zIndex: 1 }}>
                  <div style={{ width: 32, height: 32, borderRadius: "50%", background: passed ? t.color : "#181818", border: `2px solid ${passed ? t.color : "#333"}`, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: current ? `0 0 12px ${t.color}88` : passed ? `0 0 6px ${t.color}44` : "none", transition: "all 0.3s" }}>
                    {passed
                      ? <span style={{ fontSize: 13, color: "#000", fontWeight: 900 }}>✓</span>
                      : <span style={{ fontSize: 8, fontWeight: 800, color: MUTED }}>{t.wkThresh < 17 ? `W${t.wkThresh}` : `M${Math.round(t.wkThresh / 4.33)}`}</span>}
                  </div>
          ...
          <div key={trk.id} style={{ marginBottom: 16 }}>
            ...
            <div style={{
              ...
              <div style={{ flex: 1 }}>
                ...
                {/* Track-level segmented bar */}
                <div style={{ height: 5, borderRadius: 3, background: "#181818", overflow: "hidden", display: "flex", marginBottom: 0 }}>
                  {tConfPct > 0 && <div style={{ width: `${tConfPct}%`, background: tAllDone ? LIME : tColor, transition: "width 0.4s" }} />}
                  {tFailPct > 0 && <div style={{ width: `${tFailPct}%`, background: RED_BAD, transition: "width 0.4s" }} />}
                  {tPendPct > 0 && <div style={{ width: `${tPendPct}%`, background: "#2e2e2e", transition: "width 0.4s" }} />}
                </div>
            ...
            <div style={{
              ...
              {/* Section tabs */}
              <div style={{ display: "flex", borderBottom: `1px solid ${BORDER}`, background: "#181818", overflowX: "auto", WebkitOverflowScrolling: "touch", scrollbarWidth: "none" }}>
                {trk.sections.map((s, si) => {
                  const isActive = isActiveTrack && sectionIdx === si;
                  const sp = sectionProgress(trk, ti, si);
                  const confPct = sp.total > 0 ? (sp.confirmed / sp.total) * 100 : 0;
                  ...
                  <div>
                    ...
                    <div style={{
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        {/* Segmented bar in header */}
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <div style={{ flex: 1, height: 6, borderRadius: 3, background: "#181818", overflow: "hidden", display: "flex" }}>
                            {confPct > 0 && <div style={{ width: `${confPct}%`, background: allDone ? LIME : sec.color, transition: "width 0.3s, background 0.6s" }} />}
                            {failPct > 0 && <div style={{ width: `${failPct}%`, background: RED_BAD, transition: "width 0.3s" }} />}
                            {pendPct > 0 && <div style={{ width: `${pendPct}%`, background: BORDER, transition: "width 0.3s" }} />}
                          </div>
                      ...
                      {/* prev / next */}
                      <div style={{ display: "flex", gap: 5, flexShrink: 0 }}>
                        <button onClick={() => setActive([ti, Math.max(0, sectionIdx - 1)])} disabled={sectionIdx === 0} style={{ width: 28, height: 28, borderRadius: 6, border: `1px solid ${BORDER}`, background: sectionIdx === 0 ? "#111" : "#181818", color: sectionIdx === 0 ? "#333" : WHITE, cursor: sectionIdx === 0 ? "default" : "pointer", fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center" }}>‹</button>
                        <button onClick={() => setActive([ti, Math.min(trk.sections.length - 1, sectionIdx + 1)])} disabled={sectionIdx === trk.sections.length - 1} style={{ width: 28, height: 28, borderRadius: 6, border: `1px solid ${BORDER}`, background: sectionIdx === trk.sections.length - 1 ? "#111" : "#181818", color: sectionIdx === trk.sections.length - 1 ? "#333" : WHITE, cursor: sectionIdx === trk.sections.length - 1 ? "default" : "pointer", fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center" }}>›</button>
                      </div>
                    </div>
              ...
              {!isActiveTrack && (
                <div style={{ padding: "10px 16px", display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {trk.sections.map((s, si) => {
                    ...
                    const anyFail = sp.failed > 0;
                    return (
                      <button key={si} onClick={() => setActive([ti, si])} style={{ display: "flex", alignItems: "center", gap: 5, padding: "4px 10px", borderRadius: 6, background: "#181818", border: `1px solid ${anyFail ? RED_BAD + "55" : allDone ? s.color + "55" : BORDER}`, cursor: "pointer" }}>
                        <div style={{ width: 6, height: 6, borderRadius: "50%", background: anyFail ? RED_BAD : allDone ? LIME : "#3a3a3a" }} />
                        <span style={{ fontSize: 10, fontWeight: 700, color: anyFail ? RED_BAD : allDone ? LIME : "#666" }}>{s.time}</span>
                        <span style={{ fontSize: 10, color: anyFail ? RED_BAD + "aa" : allDone ? LIME + "aa" : "#444" }}>{s.label}</span>
                      </button>
...
export default function App() {
  ...
  return (
    <div style={{ background: "#0a0a0a", minHeight: "100vh", color: WHITE, fontFamily: "'Inter','Helvetica Neue',sans-serif" }}>
      ...
      {/* ── STICKY HEADER — matches other apps (Shoulder/Elbow/APRE/Hip) ── */}
      <header style={{
        background: "#101417", borderBottom: "1px solid rgba(255,255,255,0.1)",
        borderRadius: 0, position: "sticky", top: 0, zIndex: 100,
        boxShadow: "0 2px 20px rgba(0,0,0,0.8)",
      }}>
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 16px" }}>

          {/* Row 1: Logo | ACL Testing | INVOLVED toggle */}
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: 58 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, height: "100%" }}>
              {/* TRM logo — inline SVG, sized to match other apps (height 44, white) */}
              <svg viewBox="0 0 867 352" fill="currentColor" role="img" aria-label="TRM"
                style={{ height: 44, width: "auto", color: WHITE }}>
                <path fillRule="evenodd" d="M541.00,4.00 L495.00,278.50 L546.00,346.50 L561.50,345.50 L593.00,144.50 L650.00,346.50 L697.50,345.50 L785.50,144.50 L786.00,348.50 L863.50,348.50 L859.50,4.00 L776.00,5.00 L685.50,202.00 L623.50,4.00 Z M270.00,4.00 L243.00,348.50 L321.50,347.50 L332.00,212.50 L426.00,348.50 L525.50,348.50 L419.50,207.50 L458.50,185.50 L476.50,166.50 L488.50,145.50 L496.50,115.50 L497.50,84.00 L492.50,61.00 L482.50,42.00 L456.50,19.00 L424.50,7.00 L396.50,4.00 Z M344.00,66.50 L371.50,66.50 L372.00,67.50 L379.50,67.50 L380.00,68.50 L383.50,68.50 L384.00,69.50 L388.50,69.50 L389.00,70.50 L391.50,70.50 L394.00,72.50 L396.50,72.50 L397.00,73.50 L398.50,73.50 L400.00,75.50 L401.50,75.50 L408.00,82.00 L408.00,83.50 L409.00,84.00 L409.00,85.50 L410.00,86.00 L410.00,87.50 L411.00,88.00 L411.00,89.50 L413.00,92.00 L413.00,95.50 L414.00,96.00 L414.00,101.50 L415.00,102.00 L415.00,110.50 L414.00,111.00 L414.00,119.50 L413.00,120.00 L413.00,123.50 L412.00,124.00 L412.00,126.50 L411.00,127.00 L411.00,129.50 L410.00,130.00 L409.00,133.50 L407.00,135.00 L407.00,136.50 L405.00,138.00 L405.00,139.50 L396.50,148.00 L395.00,148.00 L394.50,149.00 L393.00,149.00 L392.50,150.00 L391.00,150.00 L388.50,152.00 L383.00,153.00 L382.50,154.00 L379.00,154.00 L378.50,155.00 L375.00,155.00 L374.50,156.00 L369.00,156.00 L368.50,157.00 L337.00,157.00 L336.50,156.50 L336.50,145.00 L337.50,144.50 L337.50,132.00 L338.50,131.50 L338.50,119.00 L339.50,118.50 L339.50,106.00 L340.50,105.50 L340.50,94.00 L341.50,93.50 L341.50,81.00 L342.50,80.50 L342.50,68.00 Z M9.00,4.00 L4.00,72.50 L85.00,73.00 L64.00,348.50 L141.50,348.50 L163.50,73.00 L245.50,72.50 L250.50,4.00 Z"/>
              </svg>
              <span className="trm-header-subtitle" style={{ color: "rgba(255,255,255,0.1)", fontSize: 18 }}>|</span>
              <span className="trm-header-subtitle" style={{
                fontSize: 11, fontWeight: 700, letterSpacing: "0.08em",
                textTransform: "uppercase", color: "#777",
              }}>ACL Testing &amp; Outcome Measures</span>
            </div>
                ...
                {["Left", "Right"].map(s => (
                  <button key={s}
                    ...
                    style={{
                      ...
                      cursor: "pointer", transition: "all 0.15s",
                      background: data.patient.involvedSide === s ? LIME : "transparent",
                      color: data.patient.involvedSide === s ? "#0a0a0a" : "rgba(255,255,255,0.4)",
                    }}>{s}</button>
                ))}
              </div>
            </div>
      ...
      {/* ── FLOATING SESSION BAR — split-segment groups, matches Shoulder/Elbow/APRE ── */}
      <div className="trm-fab" style={{ position: "fixed", bottom: 24, right: 24, zIndex: 200, display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <div style={{ display: "flex", alignItems: "stretch", border: `1px solid ${LIME}28`, borderRadius: 7, overflow: "hidden", boxShadow: `0 1px 6px ${LIME}0a`, opacity: saving ? 0.5 : 1 }}>
          ...
          <button onClick={handleAirDrop} disabled={saving} title="Share / AirDrop" aria-label="Share / AirDrop" style={{ padding: "7px 9px", background: LIME + "0c", color: LIME + "cc", border: "none", cursor: saving ? "default" : "pointer", display: "flex", alignItems: "center" }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={`${LIME}cc`} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 7l3-3 3 3" /><path d="M12 4v12" /><path d="M7 11H6a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-6a2 2 0 0 0-2-2h-1" /></svg>
          </button>
        </div>
